...
%   sb.solve();
	...
	function success = checkCondition0(obj, bin, items)
		success = false;
		if items.areaAll - bin.area / 4 < max(items.areas)
			% _[R(i)] = _[Q]
			obj.pack(items.id(1), bin.relPos, [0 0]);
			success = true;
			...
			if items.n == 1
				...
				binNew.size(2) = binNew.size(2) - items.sizes(1, 2);
				binNew.relPos = bin.relPos;
				binNew.relPos(2) = bin.relPos(2) + items.sizes(1, 2);
				itemsNew = sliceItems(items, 1, false);
